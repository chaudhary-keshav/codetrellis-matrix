...
| File                                                        | Purpose                                                                                 |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `codetrellis/language_config.py`                            | **Step 0 (v4.96):** Single source of truth — add `LanguageInfo` entry for new language  |
...
### Phase 0: Language Config (NEW v4.96)

- [ ] Add `LanguageInfo(...)` entry to `LANGUAGES` tuple in `codetrellis/language_config.py`
- [ ] Verify derived dicts: `BY_KEY`, `EXT_TO_LANG`, `EXT_TO_DISPLAY` include new language
...
## Step 0: Register Language in `language_config.py` (Added v4.96)

> **This step is now required before all other steps.** The centralized language registry ensures that `scanner.py`, `discovery_extractor.py`, and `init_integrations.py` automatically recognize the new language — no manual dict edits in those files.

### File: `codetrellis/language_config.py`

#### 0.1 The `LanguageInfo` Dataclass

Each language is defined as a frozen dataclass with these fields:

```python
@dataclass(frozen=True)
class LanguageInfo:
    key: str                    # Lowercase identifier (e.g., "rust", "go", "python")
    display_name: str           # Human-readable (e.g., "Rust", "Go", "Python")
    extensions: frozenset       # File extensions (e.g., frozenset({".rs"}))
    manifest_files: frozenset   # Build/project files (e.g., frozenset({"Cargo.toml"}))
    version_file: str           # File containing project version (e.g., "Cargo.toml")
    version_regex: str          # Regex to extract version from version_file
    version_bump_hint: str      # Instructions for bumping version (used in AI-generated docs)
    aliases: frozenset          # BPL/convention aliases (e.g., frozenset({"rust", "cargo"}))
    linters: tuple              # Linter tool names (e.g., ("clippy", "rustfmt"))
    type_checkers: tuple        # Type checker names, empty tuple if N/A
    test_command: str           # Default test command (e.g., "cargo test")
    test_tools: tuple           # Test framework names (e.g., ("cargo-test",))
    project_type_label: str     # Default project type (e.g., "library", "web-service")
```

#### 0.2 Add Your Entry

Add a `LanguageInfo(...)` to the `LANGUAGES` tuple in `language_config.py`:
...
LANGUAGES: tuple[LanguageInfo, ...] = (
    ...
    LanguageInfo(
        ...
        manifest_files=frozenset({"mylang.toml"}),
        version_file="mylang.toml",
        version_regex=r'version\s*=\s*"([^"]+)"',
        version_bump_hint="Edit version in mylang.toml",
        aliases=frozenset({"mylang"}),
        linters=("mylanglint",),
        type_checkers=(),
        test_command="mylang test",
        test_tools=("mylangtest",),
        project_type_label="library",
    ),
)
```

#### 0.3 What Happens Automatically

Once the entry is added, these derived dicts are rebuilt at import time:

| Dict               | How it helps                     | No manual edit needed in |
| ------------------ | -------------------------------- | ------------------------ |
| `EXT_TO_LANG`      | Maps `.ml` → `"mylang"`          | `discovery_extractor.py` |
| `MANIFEST_TO_LANG` | Maps `mylang.toml` → `"mylang"`  | `discovery_extractor.py` |
| `EXT_TO_DISPLAY`   | Maps `.ml` → `"MyLang"`          | `scanner.py`             |
| `BY_KEY`           | Maps `"mylang"` → LanguageInfo   | `init_integrations.py`   |
| `ALIAS_MAP`        | Maps `"mylang"` → LanguageInfo   | `init_integrations.py`   |
| `LINTER_LANG`      | Maps `"mylanglint"` → `"mylang"` | `init_integrations.py`   |
| `TEST_TOOL_LANG`   | Maps `"mylangtest"` → `"mylang"` | `init_integrations.py`   |

#### 0.4 Verify

```python
from codetrellis.language_config import BY_KEY, EXT_TO_LANG, EXT_TO_DISPLAY
assert "mylang" in BY_KEY
assert ".ml" in EXT_TO_LANG
assert ".ml" in EXT_TO_DISPLAY
print("✅ Language registered")
```

Then run `pytest tests/ -x -q` to confirm no regressions.

#### 0.5 Which Files You Still Need to Edit Manually

Even with the centralized registry, these files still require manual changes per language:

| File                                         | What to add                                                   | Why it can't be automated  |
| -------------------------------------------- | ------------------------------------------------------------- | -------------------------- |
| `codetrellis/extractors/<lang>/*.py`         | Extractor logic                                               | Language-specific parsing  |
| `codetrellis/<lang>_parser_enhanced.py`      | Parser class                                                  | Language-specific AST      |
| `codetrellis/scanner.py`                     | `ProjectMatrix` fields + `_parse_<lang>()` + dispatcher entry | Per-language data shapes   |
| `codetrellis/compressor.py`                  | `_compress_<lang>_*()` methods                                | Per-language output format |
| `codetrellis/bpl/practices/<lang>_core.yaml` | BPL practices                                                 | Domain-specific rules      |
...
> **Note (v4.96):** The old local `ext_to_lang` dict in `scanner.py` has been replaced by `EXT_TO_DISPLAY` imported from `codetrellis/language_config.py`. If you completed Step 0, the extension mapping is already handled. You still need to add ProjectMatrix fields, the parser import/init, the `_parse_<lang>()` method, and the dispatcher entry.
...
_Document Version: 5.3_
_Created: 2026-02-02_
_Updated: Session 76 - Phase CB: Centralized Language Configuration (v4.96) — Created `codetrellis/language_config.py` single source of truth with 32 LanguageInfo entries. Refactored `init_integrations.py`, `discovery_extractor.py`, `scanner.py` to use centralized registry. Added Step 0 to guide + Key Files table update. 7337 total tests passing_
