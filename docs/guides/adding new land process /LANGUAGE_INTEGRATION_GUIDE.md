# CodeTrellis Language Integration Guide v5.7

> **Updated:** Session 83 - React Native Framework Parser (v5.7.0) — 1 new framework-level parser (overlay on JS/TS files) + 5 extractors (component/navigation/native_module/styling/platform). ReactNativeParseResult with 18 fields. 40+ FRAMEWORK_PATTERNS. RN version detection 0.59-0.76+. Architecture detection (bridge/new_architecture/bridgeless). `is_react_native_file()` gate function. Scanner: 18 `rn_*` ProjectMatrix fields, `_parse_react_native()` dispatched in JS+TS blocks. Compressor: 5 sections (REACT_NATIVE_COMPONENTS/NAVIGATION/NATIVE_MODULES/STYLING/PLATFORM). BPL: 50 practices RN001-RN050 in `react_native_core.yaml`, `JS_REACT_NATIVE` PracticeCategory enum. A5.x: cache_optimizer (5 SEMANTIC entries), jit_context (.tsx/.jsx mappings), skills_generator (detect+context sections), mcp_server (components aggregate). 46 tests, 7264 total passing, 0 regressions. **38th reference example for framework parser integration.**
> **Updated:** Session 79 - Phase CC: Ruby Framework Parsers (v5.3.0) — 5 Ruby framework parsers (Rails 3.x-8.x / Sinatra 1.x-4.x / Hanami 1.x-2.1+ / Grape 0.x-2.x / Sidekiq 5.x-7.x), following self-contained Gin parser pattern. 5 parser files (rails/sinatra/hanami/grape/sidekiq*parser_enhanced.py), 45 dataclasses, 120+ framework detection patterns, ~52 new ProjectMatrix fields, scanner integration (5 `\_parse*_`methods in Ruby dispatch chain), compressor (5 sections [RAILS]/[SINATRA]/[HANAMI]/[GRAPE]/[SIDEKIQ] + 5`*compress*_`methods), BPL (5 practice YAMLs: 85 practices total), Scanner Evaluation on 3 repos (discourse/discourse 303 models/92 controllers/1029 routes/2269 migrations, sinatra/sinatra 120 routes/20 helpers, ruby-grape/grape 1391 endpoints/924 params), 3 bugs fixed, 42 new tests, 6927 total passing (0 regressions). **37th reference example for framework parser integration.**
**Updated:** Session 77 - v5.1.0: AI Context Quality Features — 4 cross-cutting features (Git Context extraction, Change-Frequency Sorting, JIT Dependency-Graph Boosting, Remote Repo scan). These are NOT language-specific parsers but improve matrix quality for ALL languages. New module:`git*context.py`(GitContextExtractor). Scanner: 2 new ProjectMatrix fields. Compressor: new [GIT_CONTEXT] section + sort-by-change-frequency in _compress_logic(). JIT: co-occurrence-based section boosting. CLI:`--remote URL`flag. 37 new tests, 6812 total passing.
